...
class FrontBackBackFront(CardManager):
    ...
    def save_cards(self):
        ...
        self.front_back_card = self._save_card(
            card=front_back_card,
            front=self.card_description["front"]["text"],
            back=self.card_description["back"]["text"])

        back_front_card = self.back_front_card or Card()
        self.back_front_card = self._save_card(
            card=back_front_card,
            front=self.card_description["back"]["text"],
            back=self.card_description["front"]["text"])

        self._save_metadata()

    def _save_card(self, card, front, back):
        card.front = front
        card.back = back
